...
class CTkTable(customtkinter.CTkFrame):
            ...
            for j in range(self.columns):
                ...
                if self.write:
                    ...
                    self.frame[i, j] = customtkinter.CTkEntry(
                        self, font=self.font, corner_radius=0, **args
                    )
                    self.frame[i, j].insert("0", value)
                    self.frame[i, j].bind(
                        "<Key>",
                        lambda e, row=i, column=j, data=self.data: self.after(
                            100, lambda: self.manipulate_data(row, column)
                        ),
                    )
                    self.frame[i, j].grid(
    ...
    def deselect(self, row, column):
        self.frame[row, column].configure(
